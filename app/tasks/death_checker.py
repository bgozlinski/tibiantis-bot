...
async def check_character_deaths_by_enemies():
    ...
    try:
        # Create scraper and repositories
        ...
        tasks = [
            process_character_deaths(character, enemy_names, scraper)
        ...
        if killed_by_enemies:
            ...
            await send_enemy_kills_table(killed_by_enemies)
        else:
...
async def process_character_deaths(character, enemy_names, scraper):
    ...
    deaths = await scraper.get_character_deaths_async(character.name)
    killed_entries = []
...
async def send_enemy_kills_table(killed_by_enemies: List[Dict]):
